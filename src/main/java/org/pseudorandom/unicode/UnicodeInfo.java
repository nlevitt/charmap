...
public class UnicodeInfo
{
	...
	protected LinkedHashMap<Integer,CharInfo> data;
	protected HashMap<String,UnicodeSubset> scripts;
	protected HashMap<String,UnicodeSubset> viewCache;
	protected LinkedHashMap<String, UnicodeSubset> blocks;

	private UnicodeInfo() throws IOException {
		data = new LinkedHashMap<Integer,CharInfo>();
		scripts = new HashMap<String,UnicodeSubset>();
		blocks = new LinkedHashMap<String,UnicodeSubset>();
		...
		loadBlocks(new BufferedReader(new InputStreamReader(getClass().getResourceAsStream("Blocks.txt"), "ASCII")));
		log.debug("Memory: " + Runtime.getRuntime().freeMemory()/1048576  + "M free; " + Runtime.getRuntime().maxMemory()/1048576 + "M max; " + Runtime.getRuntime().totalMemory()/1048576 + "M total");
		...
		for (String line = reader.readLine(); line != null; line = reader.readLine()) {
			...
			if (matcher.matches()) {
				...
				if (ch != lastCh && ch % 0x400 == 0) {
					log.debug("At Unihan line [" + line + "] Memory: "
							+ Runtime.getRuntime().freeMemory() / 1048576
							+ "M free; " + Runtime.getRuntime().maxMemory()
							/ 1048576 + "M max; "
	...
	protected void loadBlocks(BufferedReader reader) throws IOException {
		log.debug("loading Blocks.txt");
		
		Pattern pattern = Pattern.compile("^([0-9A-F]{4,6})\\.\\.([0-9A-F]{4,6}); (.+)");
		for (String line = reader.readLine(); line != null; line = reader.readLine()) {
			Matcher matcher = pattern.matcher(line);
			if (matcher.matches()) {
				...
				if (blocks.get(block) == null) {
					blocks.put(block, new UnicodeSubset(block));
				}

				for (int ch = ch0; ch <= ch1; ch++) {
					getData(ch).setBlock(block);
					getData(ch).setBlockIndex(blocks.get(block).getCount() + ch - ch0);
				}

				blocks.get(block).addRange(ch0, ch1);
			}
		}
	}

	private void loadScripts(BufferedReader reader) throws IOException {
		log.debug("loading Scripts.txt");

		String line = reader.readLine();
		Matcher matcher = Pattern.compile("# Scripts-([0-9.]+)\\.txt").matcher(line);
		if (matcher.matches()) {
			this.unicodeVersion = matcher.group(1);
		}
	...
	private void initAlgorithmicCharDetails() {
		...
		for (int ch = 0xac00; ch <= 0xd7a3; ch++) {
			...
			// log.debug("U+" + getHexString(ch) + " " + getData(ch).getName() +
			// " LIndex=" + LIndex + " VIndex=" + VIndex + " TIndex=" + TIndex);
	...
	private UnicodeSubset getView(String view) {
		...
		if (split[2].length() != 0) {
			...
			for (String range : ranges) {
				...
				try {
					...
					for (int ch = start; ch <= finish; ch++) {
						// log.debug("ch=0x" + UnicodeInfo.getHexString(ch) +
			...
			for (CharInfo detail : data.values()) {
				// log.debug("detail.getCh()=" + getHexString(detail.getCh()) +
				// " detail.getScript()=" + detail.getScript() +
				// " detail.getGC()=" + detail.getGC());
	...
	public List<SearchResult> search(String query) {
		...
		if (query.charAt(0) > 0x7f && data.get(query.codePointAt(0)) != null) {
			...
			SearchResult result = new SearchResult(results.size(),
					query.codePointAt(0), info.getScript(),
					info.getScriptIndex(), info);
			results.add(result);
		} else if (query.matches("(?i)u[+][0-9a-f]{4,6}")
				&& data.get(Integer.parseInt(query.substring(2), 16)) != null) {
	...
	public SearchResult search(int ch) {
		...
		SearchResult result = new SearchResult(0, ch, info.getScript(),
				info.getScriptIndex(), info);
		return result;
	}
	...
	public Collection<UnicodeSubset> getBlocks() {
